...
| ⬜ | 백엔드 프로젝트 초기화 | - | TBD |
| ⬜ | 프론트엔드 프로젝트 초기화 | - | TBD |
| ⬜ | Docker Compose 설정 | 컨테이너 정상 실행 확인 | TBD |
| ⬜ | CI/CD 파이프라인 설정 | 파이프라인 정상 동작 확인 | TBD |

### 1.2 데이터베이스 설정

| 상태 | 태스크 | 테스트 케이스 | 담당자 |
|:----:|--------|---------------|--------|
| ⬜ | PostgreSQL 스키마 설계 | - | TBD |
| ⬜ | Alembic 마이그레이션 설정 | 마이그레이션 실행/롤백 테스트 | TBD |
| ⬜ | Redis 연결 설정 | 연결 테스트 | TBD |
| ⬜ | Elasticsearch 인덱스 설정 | 인덱스 생성 테스트 | TBD |

---

## 🔴 Phase 2: 인증 서비스 (Auth Service)
